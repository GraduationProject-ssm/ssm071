...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1T48XecE9G?p=69)
...
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此信息管理系统为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
图4.1 程序工作的原理图
## 4.2 系统功能结构设计
在分析并得出使用者对程序的功能要求时，就可以进行程序设计了。如图4.2展示的就是管理员功能结构图，管理员主要负责填充知识库和其类别信息，并对已填充的数据进行维护，包括修改与删除，管理员也需要查看考勤和项目管理等功能。
...
|3|today|String|打卡日期|是|
|4|on\_time|Date|上班打卡时间|是|
|5|down\_time|Date|下班打卡时间|是|
|6|chuqin\_types|Integer|打卡类型|是|
|7|overtimeNumber|Integer|加班时间(小时)|是|
表4.2字典表表
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|13|yonghu\_id|Integer|项目负责人|是|
|14|xiangmu\_name|String|项目名|是|
|15|xiangmu\_types|Integer|项目类型|是|
|16|xiangmujindu\_types|Integer|项目进度|是|
|17|xiangmu\_content|String|项目详情|是|
|18|xiangmu\_file|String|项目文件|是|
|19|insert\_time|Date|项目开始时间|是|
|20|update\_time|Date|项目结束时间|是|
表4.4系统公告表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|21|addtime|Date|创建时间|是|
|22|biaoti|String|标题 Search|是|
|23|leixing|String|类型|是|
|24|neirong|longtext|内容|是|
|25|riqi|datetime|日期|是|
表4.5用户表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|26|username|String|账户|是|
|27|password|String|密码|是|
|28|name|String|姓名|是|
|29|phone|String|手机号|是|
|30|id\_number|String|身份证号|是|
|31|sex\_types|Integer|性别|是|
|32|age|Integer|年龄|是|
|33|bumen\_types|Integer|部门|是|
|34|zhiwei\_types|Integer|职位|是|
|35|my\_photo|String|照片|是|
|36|nation|String|民族|是|
|37|politics\_types|Integer|政治面貌|是|
|38|birthplace|String|籍贯|是|
表4.6知识库表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|39|zhishi\_name|String|知识文件名|是|
|40|zhishi\_types|Integer|知识类型|是|
|41|zhishi\_content|String|文件简介|是|
|42|zhishi\_file|String|知识文件|是|
|43|insert\_time|Date|上传时间|是|
表4.7用户表表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|44|username|String|用户名|是|
|45|password|String|密码|是|
|46|role|String|角色|是|
...
## 5.1 管理员功能介绍
### 5.1.1 用户管理
如图5.1显示的就是用户管理页面，此页面提供给管理员的功能有：新增用户,修改用户,删除用户修改用户信息。

![](/md/blog.010.png)

图5.1 用户管理页面
### 5.1.2 公告管理
如图5.2显示的就是考勤管理页面，此页面提供给管理员的功能有：查看考勤打卡jil;。

![](/md/blog.011.png)

图5.2 公告管理页面
### 5.1.3 项目管理
如图5.3显示的就是项目管理页面，此页面提供给管理员的功能有：新增项目,修改项目信息,查看和修改项目信息,修改项目状态等功能。

![](/md/blog.012.png)

图5.3 项目管理页面
### 5.1.4 系统公告管理
如图5.4显示的就是系统公告管理页面，此页面提供给管理员的功能有：新增公告,删除公告.修改公告内容。
